...
detector behaves exactly as `BeiDouB1I`'s — one NH20 period is one
...
soft `_detect_secondary_code_cfar` and this method is reached only
if a caller forces B3I onto the hard path; the per-PRN packed reference
comes from the generic `_packed_secondary_code` either way. Returns
[`SyncResult`](@ref).
"""
